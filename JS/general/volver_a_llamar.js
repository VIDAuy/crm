...
function agendar_volver_a_llamar(openModal = false) {
    if (openModal === true) {
        ...
        let hora = $("#hora_nueva_agenda_volver_a_llamar").val();
        ...
        if (fecha == "") {
            ...
            error("Debe ingresar una hora");
        } else if (mensaje == "") {
            error("Debe ingresar un mensaje");
        } else {
            $.ajax({
                type: "POST",
                url: `${url_app}agendar_volver_a_llamar.php`,
                data: {
                    area: area,
                    cedula: cedula,
                    telefono1: telefono1,
...
function cantidad_volver_a_llamar() {
    ...
    if (area == "Morosos" || area == "Calidad" || area == "Bajas") {
        $.ajax({
            ...
            success: function (response) {
                if (response.error === false) {
                    document.getElementById("cantidad_pendientes_volver_a_llamar").innerHTML = response.cantidad + "+";
                }
            },
        });
    }
...
function recordatorio_volver_a_llamar() {
    let area = $("#sector").val();
    let id_sub_usuario = localStorage.getItem("id_sub_usuario");
    ...
    if (openModal === true) {
        ...
        if (observacion == "") {
            ...
            $.ajax({
                type: "POST",
                data: form_data,
                url: `${url_app}cargar_registro_volver_a_llamar.php`,
                dataType: "JSON",
                contentType: false,
                processData: false,
                beforeSend: function () {
...
function tabla_llamadas_pendientes() {
    ...
    $('#tabla_llamadas_pendientes').DataTable({
        ajax: `${url_app}tabla_llamadas_pendientes.php?cedula=${cedula}&area=${id_sector}`,
        columns: [
            { data: 'id' },
            { data: 'cedula' },
            { data: 'nombre' },
...
function abrir_asignar_llamada(openModal = false, id, id_area, cedula, nombre, telefono, es_socio, baja, fecha_hora, mensaje, fecha_registro, id_usuario_asignado = null, usuario_asignado = null, id_usuario_asignador = null, usuario_asignador = null) {
    if (openModal === true) {
        ...
        if (id_registro == "" || id_usuario_asignador == "" || tipo_operacion == "") {
            ...
            $.ajax({
                type: "POST",
                url: `${url_app}asignar_llamada_a_usuario.php`,
                data: {
...
function select_sub_usuarios(opcion, div, id_sub_usuario = null, nombre_sub_usuario = null) {
    ...
    if (opcion == "Agregar") {
        ...
        $.ajax({
            type: "GET",
            url: `${url_app}select_sub_usuarios.php?area=${id_area}`,
            dataType: "JSON",
            success: function (response) {
        ...
        $.ajax({
            type: "GET",
            url: `${url_app}select_sub_usuarios.php?id_sub_usuario=${id_sub_usuario}`,
            dataType: "JSON",
            success: function (response) {
                let datos = response.datos;
                datos.map((val) => {
